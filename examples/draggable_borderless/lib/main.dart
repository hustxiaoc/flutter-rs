...
class DraggebleAppBar extends StatelessWidget implements PreferredSizeWidget {
  static const platform_channel_draggable = MethodChannel('flutter-rs/window');
  ...
  @override
  Widget build(BuildContext context) {
    return GestureDetector(child: appBar, onPanEnd: onPanEnd, onPanStart: onPanStart, onPanUpdate: onPanUpdate);
  }
...
//    await platform_channel_draggable.invokeMethod('onPanUpdate');
  ...
  void onPanStart(DragStartDetails details) async {
    await platform_channel_draggable.invokeMethod('start_drag');
  }

  void onPanEnd(DragEndDetails details) async {
    await platform_channel_draggable.invokeMethod('end_drag');
  }
}
